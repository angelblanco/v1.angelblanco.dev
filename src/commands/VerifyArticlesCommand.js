...
module.exports = class VerifyArticlesCommand extends BaseCommand {
    ...
    async action() {
        ...
        this.log('');
        this.log(`${this.chalk.gray('Verifying articles matching:')} ${this.chalk.yellow(mdFileGlob)}`);
        this.log('');

        const mdFiles = await this.globby(this.articlePath('*.md'));

        const promises = mdFiles.map((file, index) => this.verifyMdFile(file, index));
    ...
    async verifyMdFile(path) {
        ...
        if (!attributes.date
            ...
            error(`The article must have a valid logical date, given ${attributes.date}`);
        }

        if (attributes.noTags !== true && (!attributes.tags || attributes.tags.length === 0)) {
    ...
    async verifyIncrementalIdDates() {
        ...
        keys.forEach((id, index) => {
            ...
            if (nextDate.diff(date, 'days') < 0) {
                throw new Error(`The article with id "${nextId}" has a lower date than the prev article ${id}`);
            }
        });
    }
}
