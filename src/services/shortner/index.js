...
export function intAsString(value) {
    ...
    if (!strValue.match(/^[1-9][0-9]*$/)) {
        throw new Error('The provided value is not an base 10 integer or an string integer greater than 0');
    }
...
export function shortner(preffix, id) {
    ...
    const zerosToFill = 0 === initalJoinSting.length % 3
        ? ''
...
export function reverseShortner(str) {
    const err = (msg) => {
        throw new Error(`Unable to reverse shortner the provided id "${str}". ${msg}`);
    };
