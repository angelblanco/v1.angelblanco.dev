...
function validateMeta(meta, id) {
  ...
  if (meta.published !== false) {
    ...
    if (meta.noTags !== true && !meta.tags) {
      throw new Error(
        `All published articles must have at least one tag @ ${id}`
      );
    }

    if (!meta.intro) {
      throw new Error(
        `All published articles must have an introduction @ ${id}`
      );
    }
  }
}
...
export function createMarkdownItInstance(options = {}) {
  ...
  containers.forEach((registerContainer) =>
    registerContainer(mdInstance, markdowContainer)
  );
...
export function createJsonToc(html, id) {
  ...
  const titles = Array.from(
    document.querySelectorAll('h1, h2, h3, h4, h5, h6, h7')
  );
    ...
    if (!lastStackItem) {
      ...
      if (prevTreeItem) {
        if (item.level !== prevTreeItem.level) {
          warn(
            `The root titles are not in tree structure on markdown file: ${id} on header "${item.title}`
          );
        }
      ...
      if (lastStackItem.level !== item.level - 1) {
        warn(
          `The titles are not in tree structure on markdown file: ${id} on header "${item.title}`
        );
      }
...
export function parseCode(code, id = '', md) {
  ...
  const slug = !meta.slug || meta.slug === 'auto' ? autoSlug(id) : meta.slug;
